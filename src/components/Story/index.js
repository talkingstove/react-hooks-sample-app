...
export default function Story(props) {
  ...
  return (
    <li key={storyData.id} className="story-list-item">
      <h4>
        <a href="javascript:;" onClick={() => props.showThreadView(storyData)}>
          {storyData.title}
        </a>
      </h4>
      <div>
      ...
      <div className="author">by: {storyData.by}</div>
      <div className="time">
        at: {new Date(storyData.time * 1000).toString()}
      </div>
      <div className="score">score: {storyData.score}</div>
      <div className="score">comments: {comments}</div>
    </li>
  );
}
